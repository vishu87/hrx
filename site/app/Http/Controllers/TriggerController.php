...
class TriggerController extends Controller {
    ...
    public function getTriggerList(){
        $triggers = DB::table('aims_triggers')->select('id','trigger_name','parent_id')->get();

        $categories = DB::table('aims_trigger_categories')->select('id','name','parent_id')->get();
    ...
    public function saveUserTrigger(){
        ...
        DB::table('aims_user_triggers')->where('user_id',Auth::user()->parent_user_id)->where('active',1)->update(array(
            "active" => 0
        ));
